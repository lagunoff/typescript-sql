...
const parseTuple = str => {
  ...
  for(;;) {
    ...
    if (str[iter] === '{' || str[iter] === '[') {
      ...
      if (str[iter] === '[') {
        values.push(dsl.optional(parseExpr(substr)));
      } else {
    ...
    if (str[iter] === '<') {
      ...
      values.push(dsl.ref(prepareRuleName(name)));
      iter += name.length;
      continue;
    }
    if (/^\s$/.test(str[iter])) {
      ...
      if (name === '...') {
        values.push(dsl.many1(values.pop()));
      } else {
        values.push(dsl.of(name));
      }
      continue;      
    }
  }
  if (values.length === 1) return values[0];
  return dsl.tuple(...values);
};
...
const parseRule = s => {
  ...
  return dsl.rule(prepareRuleName(name), expr);
};
...
function debugInfo(components, gp, gpinv) {
  Object.keys(components).forEach(c => {
    ...
    console.log(dsl.getComment(rules[c]))
